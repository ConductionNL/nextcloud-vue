...
| `connectionStatus` | Renders a connection registry status as its label: `configured` is Configured, `limited` is Limited, `unconfigured` is Not configured, `simulated` is Simulated, `unavailable` is Not available, `error` is Error. Any other value passes through unchanged. Translated through the library catalogue, Dutch included. |
