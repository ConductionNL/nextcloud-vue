...
| `connect`            | —       | A new connection was made, by pointer OR by keyboard. Carries Vue Flow's `{ source, target, sourceHandle, targetHandle }`, with the handles reduced to the PORT the host declared — see `portIdOf` below.                                                     |
