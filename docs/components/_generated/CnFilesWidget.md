...
| `apiBase`   | `string`  |          | `'/apps/files'` | App base for the host's files-widget endpoints (`{apiBase}/api/widgets/files/{placementId}/...`). Lets a consuming app point the widget at its own backend (e.g. `/apps/mydash`). Defaults to `/apps/files`. |
