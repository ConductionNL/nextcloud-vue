...
| `page.config.createOverride` (type=`index`) | A **string** naming an async create handler. Resolved across both registries: a v2 `cnRegistry` entry with `kind:'create-override'` exposing the function as `.handler` (or `.fn`), a directly function-valued registry entry, or a function-valued legacy `customComponents` entry. The resolved function is forwarded to `CnIndexPage`'s [`createOverride`](./cn-index-page.md#per-schema-create-override-hook) prop, so a create from the generic Add button runs the app's side-effecting persistence path (e.g. a contact-aware endpoint filling a required FK) instead of saving straight to OpenRegister. An unresolved name is dropped with a one-line `console.warn`. |
