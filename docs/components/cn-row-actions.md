...
The tables below are generated from the SFC source via `vue-docgen-cli`. They reflect what's actually in [`CnRowActions.vue`](https://codeberg.org/Conduction/nextcloud-vue/src/branch/beta/src/components/CnRowActions/CnRowActions.vue) and update automatically whenever the component changes.
