...
Force Refresh on/off per-instance with `:show-refresh="true"`/`:show-refresh="false"` (the legacy `hide-refresh` alias still opts out for back-compat); opt out of Request-a-feature with `:show-request-feature="false"`. When everything is hidden — Refresh auto-hidden or opted out, Request-a-feature opted out, no `documentationUrl`, and no `#action-items` slot content — the overflow menu disappears entirely.
