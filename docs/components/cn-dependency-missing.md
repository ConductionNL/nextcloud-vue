...
- **Admin** (`getCurrentUser().isAdmin`): an **Install and enable** / **Enable** button driven by [`useAppInstaller`](../utilities/composables/use-app-installer.md). One click confirms the admin's password, then downloads-installs-and-enables the app via Nextcloud's `settings/apps/enable` endpoint, and reloads on success. On failure the error shows inline and the store link (`resolveLink(dep)`) stays as a fallback.
