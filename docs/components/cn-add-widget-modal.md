...
| `file-upload-fn` | `Function` \| `null` | `null` | Optional raw-file upload transport forwarded to the active sub-form as its `upload-fn`: `async (file: File) => ({ url })`. Distinct from `upload-fn` (icon picker, data URL). Sub-forms such as the image widget defer the upload to submit and hand over the raw `File`; the modal awaits the sub-form's `commit()` before emitting `submit`. |
