...
| `inlineActions` | Number \| null | `null` | How many header actions stay as buttons; the rest collapse into one `···` menu (forwarded to [CnActionButtons](./cn-action-buttons.md)' `inline`). Setting it **also folds this page's own Edit button into that cluster**, so one count governs the whole header instead of Edit sitting outside the limit. `null` keeps both of today's behaviours — every action as a button, Edit standalone. |
