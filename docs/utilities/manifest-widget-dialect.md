...
| 2.4 | `promoteCustomDashboard` | `src/cli/transforms/promoteCustomDashboard.js` | Promotes `type:"custom"` pages whose component is a known bespoke dashboard (`Dashboard`, `DashboardIndex`, `DashboardView`, `DashboardCustomView`, `ScholiqDashboards` — the literal component name shipped in Learniq's manifests, matched as data and so left unrenamed) to `type:"dashboard"`. Bespoke logic with no manifest-widget expression is flagged (component name preserved), never silently dropped. |
