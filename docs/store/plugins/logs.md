...
| `autoRefreshOnItemChange` | `boolean` | `false` | When `true`, overrides `setItem` so selecting an item automatically refreshes logs (and clearing the item clears logs). |
...
| `setItem` | `(data) => void` | **Only when `autoRefreshOnItemChange: true`** — overrides the base `setItem`: wraps in the configured `Entity`, then calls `refreshLogs()` when the new item has an id, or `clearLogs()` when cleared. |
...
- **Overriding**: `extend.actions` runs *after* plugins, so you can still override `refreshLogs` / `setItem` in `extend` if the default behavior doesn't fit.
