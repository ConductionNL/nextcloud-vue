	...
	if (!entry) {
		...
		function runCapture(step, source) {
			...
			for (const [varName, token] of Object.entries(cap)) {
				...
				let value
				if (source.params && key in source.params) { value = source.params[key] } else if (source.object) { value = (key === 'id') ? (source.object.id ?? source.object['@self']?.id ?? source.object.uuid) : source.object[key] }
