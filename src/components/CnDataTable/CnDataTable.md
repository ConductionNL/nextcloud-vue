...
| `sortKeys` | Array | `[]` | Ordered multi-column sort state (`[{ key, order }]`, up to 3). Empty → falls back to single-sort `sortKey`/`sortOrder`. Shift+click a header to append a key. |
