...
export default {
	...
	computed: {
		...
		rowVariant() {
			...
			if (!resolve || !resolve.variantField || !this.referenceRow) return ''
		...
		badgeVariant() {
			...
			for (const candidate of candidates) {
				...
				if (BADGE_VARIANTS.includes(variant)) return variant
			}
			return 'default'
		},
		/**
		...
		variantColor() {
			...
			const override = this.activeOverride
			if (override && override.variant) return VARIANT_COLORS[override.variant] || ''
			const rule = this.activeVariantRule
			...
			if (this.rowVariant && VARIANT_COLORS[this.rowVariant]) return VARIANT_COLORS[this.rowVariant]
	...
	methods: {
		 ...
		 * @param {{field: string, op?: string, value?: *}} when The condition.
		 * @return {boolean} True when the override applies.
		 */
		matchesOverride(record, when) {
			if (!when || typeof when !== 'object' || typeof when.field !== 'string' || when.field === '') return false
			const compares = Object.prototype.hasOwnProperty.call(when, 'value') || Boolean(when.op)
			if (!compares) return Boolean(readVisibleWhenPath(record, when.field))
