...
- **The stage strip on a record is now a widget, so the person who owns the page can place it, resize it or take it off.** dossiq's case page carried a hand-written transition strip across a whole row. Nobody but a developer could change it, it could not be moved, and the next app that wanted one had to write it again. `CnStagesWidget` is registered as the `stages` type on the detail-page surface, configured from the page itself by `CnStagesWidgetForm`.
  ...
  It reads the current stage off the bound record and the stage list from either an app endpoint or an OpenRegister query, both through the shared endpoint engine, so two widgets reading one endpoint still issue one request. Clicking a reachable stage moves the record, either by saving its stage property or by calling a transition endpoint with the move id. Nothing about the record type is hard-coded: a case, a request and a deal use the same widget with different config.

  An optional availability endpoint gates each move. A blocked stage renders disabled with its reason on screen, because the reason is what tells the person what the record still needs, and it keeps its focus stop with `aria-disabled="true"` so a keyboard reaches it and hears why it is closed. A move that declares a comment or a result opens `CnStageMoveDialog` before anything is sent, and the confirm button stays disabled until what the move requires is there. The widget re-checks the same requirement where the request is made, because a guard that lives only where the button is drawn is not enforced on the path that sends.

  **The guard fails closed, in every state where it cannot be trusted.** `allowedField` blocks on `false`, `0`, `'0'` and `'false'`, listed rather than inferred: a JSON round trip or a database column produces the string where the schema said boolean, and recognising only the literal `false` would open a stage the server had closed. An availability read that failed blocks every move, and so does an `availability` block that is present but names no url, because a typo in the key is a broken guard rather than an absent one. After a move the strip stays blocked until the fresh answer arrives, since the map in hand describes the stage the record has just left.

  After a move the widget shows the new stage at once and fires `cn:page:refresh`, so the page re-reads the record and the availability answer for the new stage is refetched rather than assumed. The re-read record is authoritative whatever it says, so a server that accepted the call and left the status alone no longer leaves the strip claiming a stage the record never reached. `CnTimelineStages` learns a per-stage `disabled` flag for this, and emits no `stage-click` for one.
