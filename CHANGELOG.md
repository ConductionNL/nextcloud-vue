...
### Fixed
- **An `ncDashboard` id is unique across the whole manifest, and the schema version says the key exists (2.40.0 → 2.41.0).** `ncDashboard` was added without a version bump, and the schema version is the only machine-readable signal a fleet app has that its vendored copy is stale — the package version is not one, it reads `2.0.5` at every release tag — so a consumer comparing versions saw nothing move and had no reason to re-vendor. Two rules land with the bump. The id must now be unique across every page's `widgets[]`, not merely within one: the schema required the id so a panel would not hang off array position, but Nextcloud keys each user's chosen dashboard widgets **per app**, so two pages both declaring `id: "recent"` derive one panel and whichever the host registers second silently takes the first one's place — the same one-way surprise the requirement was written to prevent, one scope up. Manifest-wide uniqueness is beyond JSON Schema, so `validateManifestV2` enforces it post-schema, beside the `pages[].id` rule it already ran. And `ncDashboard.link` was an unconstrained string described only as "in-app route", leaving every host to guess whether it had been handed a route or a URL; it now takes the spelling `pages[].route` uses and refuses a scheme (`javascript:` above all) and a protocol-relative `//host`. **Consumer impact:** no shipped manifest declares `ncDashboard` yet, so nothing in the fleet changes; an app adopting it needs the id unique app-wide and the link relative.
- **A retargeted object list stops printing property names at the reader again.** Column headings read the schema the create dialog caches, and the `content.register` / `content.schema` watchers drop that cache on a retarget — correctly, since a schema resolved for the old register is the wrong-app schema the register scoping exists to prevent. Only the dialog had a later moment that refilled it. The headings did not, so a widget `CnRelatedCollections` reuses (it keys children by index, so a reordered entry lands on a live component) went straight back to `customerName` and `dateReceived` and stayed there until somebody happened to open Add — which is the bug the headings feature was written to end, returning by the one path it had not been tested on. Both watchers now reload the headings after dropping the cache.
- **`CnCapabilityTable` gives a reader their own words back.** `t()` escapes its placeholders by default and the result is rendered through `{{ }}`, which escapes nothing further — so a search for `zaken & dossiers` that matched nothing answered "No capability matches zaken &amp; dossiers", and every table caption carried an area named "Zaken & Documenten" as "Zaken &amp; Documenten", which is what a screen reader reads out to say which table it has reached. Both now pass `escape: false`, Vue's own interpolation being the escape. Three smaller repairs alongside: the search index carries a provider's `name_nl` as well as its `name`, so a Dutch reader can find a row by the Dutch provider name their own page is showing them — every other label already indexed both languages, and the module's docblock said the provider did too; a `groupBy` naming neither grouping keeps the grouping toggle instead of silently removing the control and falling back to the default anyway; and rows are keyed by position rather than by `row.id`, so a document that repeats an id or omits one renders like every other malformed shape this table promises to survive.
- **The features-and-roadmap view moves a reader off a stop that has gone away.** A capability comparison that arrives late or is cleared left `activeView` on `capabilities`, where the panel's `v-else-if` still matched and rendered an empty table under a "Capabilities" heading, on a toggle that no longer admitted the stop existed. The view now returns the reader to Features when the comparison goes; a reader on Roadmap stays there, that stop being unaffected.
- **The folder pane keeps every folder on screen while one is selected.** A `field`-source folder sidebar (`CnIndexPage`'s `folderSidebar`) lists the grouping field's facet buckets, and selecting a folder filters the query by that field, so the facet OpenRegister computed for the narrowed query held the selected value alone and every other folder vanished; switching folders meant going back to "All" first. The pane now remembers the folder set it saw with no folder selected and shows that while one is, following the live facet again once the selection is cleared. A folder selected before any facet arrived, as on a deep link, still shows what the live facet says.
- **A saved view stays on the pages it was made for.** `CnIndexPage` listed every view OpenRegister returned on every index page that allows saved views, so a view saved on a cases index was offered on the organisations index, where its filters name fields that schema does not have and applying it does nothing. A view now carries the page's register and schema in its `query` (`scope`, plus OpenRegister's own `registers` and `schemas` lists), and a page offers only the views whose scope is its own: every page over the same register and schema shares them, no other page sees them. The new `savedViewsScope` prop overrides the default, to share views across pages over different sources or to keep two pages over one source apart. Views saved before this change carry no scope and stay visible everywhere, so nobody loses one; re-saving a view pins it to its page.
- **A navigation group opened for the page you landed on stays open.** `CnAppNav` expanded the group holding the active route, then collapsed it again the moment you navigated to a page outside it, because the expansion was read off the route on every render and nothing recorded that the group had been opened. The group is now pinned open when the route enters it and stays that way until you close it yourself. A group you closed while on one of its children stays closed as you move between them; it reopens only when you come back into it from outside.
- **A grid item can size to its content, so a widget that turns out to be empty costs no row.** `CnDashboardGrid` layout items accept `sizeToContent` (`true`, or a number as a row minimum), passed through to GridStack per item. It exists for the widget that decides AT RUNTIME it has nothing to show — a banner whose condition is false, a list that came back empty. Until now that widget still cost its whole authored row, and there was no fixing it where it showed: the row is reserved from the layout before the component renders, and the engine positions items absolutely from `gridY`/`gridHeight`, so collapsing an empty one in CSS hides it without moving anything below it. Opt-in per item, because it makes `gridHeight` advisory for that widget — right for anything whose height is whatever it turned out to be, wrong for anything with a deliberate size. **Consumer impact:** none; items that do not set it are unchanged.
- **The split pane keeps the slot components the detail page declares.** `CnPageRenderer` mounted the pane's page with no slots forwarded — the slot loop feeds the index component only — so a `type: "custom"` widget, which resolves through its host page's `slots` map and nothing else, drew an empty grid cell. The same widget renders on its own route, so a record silently lost content purely by being opened beside the list instead of from it; on a dossiq case that was an 8x6 hole beside Terms and payment. The pane now resolves the DETAIL page's own `slots` (not the index's, which is a different page's set). **Consumer impact:** none beyond the fix.
- **A detail page's body grid reflows on its own width, so it is readable in a split pane.** `CnDetailPage` mounted its grid at a fixed 12 columns. In the ~520px split pane beside an index list that is ~43px per column, so a `gridWidth: 3` panel is ~130px and its own title no longer fits — two panels side by side end up unreadable while the viewport still says "desktop". A new `columnOpts` prop defaults to ONE threshold: the authored 12 columns above 1000px, a single stacked column at or below it; pass `null` for the old fixed grid. There are deliberately no steps in between, because `moveScale` rescales the authored geometry and an intermediate count lands widgets at fractional positions nobody placed — a `gridWidth: 3` tile becomes 1 of 4, a `gridWidth: 9` panel 3 of 4, left/centre/right by rounding — which reads as broken rather than narrow. Two details make it work where the existing helper would not: `breakpointForWindow` is **false**, so GridStack measures the grid element rather than `window.innerWidth` (a narrow pane on a wide window is exactly the case that matters, and `getDashboardColumnOpts()` gets this wrong for it); and `columnMax` is set explicitly, because GridStack only defaults it for a single-entry breakpoint table and otherwise assigns `undefined` above the top breakpoint, where `column(undefined)` silently no-ops and a grid that once narrowed never widens again. A reflow is NOT a layout edit: `CnDashboardGrid` drops the `change` GridStack fires while it is rescaling to another column count, so a narrow window cannot rewrite the authored geometry. Both editing lanes follow that rule rather than contradicting it — the drag and resize handles and the keyboard lane's tab stops, help text and arrow keys are all taken away while the grid is reflowed and given back as it widens, because an edit mode that accepted a move and then silently snapped it back is the same surprise pointed the other way, and harder to diagnose since the author did nothing wrong. It is worst on the keyboard, where the move was also spoken as done: a screen-reader user heard "column 2, row 3" for a move the reflow guard then dropped, on the path with the least other signal that anything had gone wrong. Enter still activates a widget there, because opening one is not repositioning it. **Consumer impact:** detail pages narrower than ~1200px now reflow instead of squeezing 12 columns; a full-width page is unchanged. An author in edit mode below the breakpoint can no longer move a widget by pointer or by keyboard, which is what the grid was already doing with the move.
- **A widget placing another app's integration leaf costs no grid cell when that app is absent.** A cross-app feature is normally optional — dossiq places humaniq's hours leaf on a case, and humaniq is not a dossiq dependency. With the leaf unregistered nothing resolved, but `CnDetailWidgetHost` still rendered its wrapper, so the missing app cost a full-height ghost holding a quarter of the case's top band on every install without it. `CnDetailPage` now drops the layout item instead. Scoped deliberately: a widget declaring `requiredApp` is KEPT, because the host answers a missing app with an actionable set-up state and that is content; and only `integration` widgets are judged at all — a `custom` widget whose `#widget-<id>` slot is missing also draws an empty host, but that is a wiring fault to surface rather than an optional feature to hide. Nothing is hidden in Buildiq edit mode, where an author has to see a placement to move or delete it. **Consumer impact:** a page placing a leaf from an app that is not installed loses that cell; the remaining items keep their declared `gridX`/`gridWidth`, so the freed space stays empty until the layout is rebalanced.
- **The Cases-style map view clusters its markers, so rows at one address stay reachable.** `CnIndexPage` never passed `clustering` and `CnMapWidget` defaults it off, so coincident markers landed on identical pixels: the topmost took every click and the rest were unreachable, with nothing on screen saying they existed. Several permits on one building is the normal case for an index map, not an edge case. It is now on unless `mapConfig.clustering` is `false` — the opposite of the widget's own default, because a widget is embedded deliberately while an index map plots whatever the filter returns. markercluster spiderfies a cluster whose children never separate, which is exactly the identical-coordinates case, and non-point geometry skips the cluster group, so polygons are unaffected. **Consumer impact:** an index map that opted into `viewModes: ["map"]` now clusters; pass `mapConfig.clustering: false` for the old behaviour.
- **A detail page fills its split pane instead of shrinking to fit and centring.** `.cn-index-page__split-pane` is a column flex container, so `.cn-detail-page` is a flex item whose cross axis is horizontal — and its own `margin-inline: auto` disables stretch there (an auto cross-axis margin does), leaving a shrink-to-fit box centred in the pane with the record squeezed into the middle of it. On the page's own route it is a block in a block, where that margin is a silent no-op, which is why it only ever showed beside a list. Fixed at the pane rather than on `.cn-detail-page`, whose auto margin still does its real job centring the page under its `maxWidth` cap on a wide screen. The header's 56px indent, which clears the Nextcloud navigation toggle at the edge of `.app-content`, is also dropped in the pane — but not below the page's breakpoint, where the pane IS the content area and the toggle really is over that edge. **Consumer impact:** none beyond the fix; the full-page route is untouched.
- **Map tiles load instead of painting "App is not following the tile usage policy".** Nextcloud sends `Referrer-Policy: no-referrer` on every page (`lib/private/../OC.php`), Leaflet draws a tile as a plain `<img>`, and OpenStreetMap's CDN answers a refererless tile request with a blocked-tile image and an `x-blocked: Access denied` header. It is served as a normal **200 PNG**, not an error, so the map painted a grey grid of policy notices rather than failing loudly — which is why it reads as an empty map and why the network log shows no obvious fault. Verified against the live CDN: the identical request with any `Referer`, including `http://localhost/`, is not blocked, and without one it is. `CnMapWidget` now defaults `referrerPolicy` to `strict-origin-when-cross-origin` on every tile layer it builds — configured basemaps, the OSM fallback, and `tile` / `wms` entries in `layers`. That value sends the instance origin but never the path, which on a case-management app is where a record id would sit. A consumer that wants something else, such as `no-referrer` for an internal tile server, still wins: the default only applies when `options.referrerPolicy` is unset. **Consumer impact:** third-party tile hosts now receive the instance's origin, which is what their usage policies require in order to serve the tiles at all. Note the CSP trap this does *not* change: an app allowlisting `https://*.tile.openstreetmap.org` must keep the `{s}` subdomain URL form, because a CSP wildcard does not match the bare `tile.openstreetmap.org`.
- **A gesture that moved the map is what hands the view over, not any gesture at all.** `CnMapWidget` stops re-framing on a marker reload once the reader has framed the view themselves, and it read that off raw input on the container (`mousedown` / `touchstart` / `wheel` / `keydown`) because Leaflet's own `movestart` fires for `fitBounds` too. Half of those change nothing, though: with `scrollWheelZoom` off, scrolling the page past the map is a wheel event on the container, and tabbing through it is a keydown — and the flag is one-way, so a single stray scroll on page load meant a marker reload never re-framed again for the rest of the session. A gesture now only ARMS the hand-over; Leaflet's `moveend` / `zoomend` commits it, and a size re-measure the widget does itself disarms it first so its own fit cannot count as the reader's. **Consumer impact:** none beyond the fix; a real pan, zoom or keyboard pan still wins, and the Fit all markers control is unchanged.
- **A detail page whose widgets arrive after mount still subscribes to presence.** `CnDetailPage` read `widgets` once, in `setup`, to decide whether a presence row was placed — so a manifest-driven page, whose widgets arrive with the manifest, always answered "no" and never subscribed. `layoutItemCanDraw` then dropped the presence row permanently, turning "nobody is here right now" into "this row never comes back". The decision is a getter now, and `useObjectPresence`'s `enabled` option accepts a boolean, a ref OR a getter and re-reads it, starting the beat when it flips. **Consumer impact:** none; a plain boolean behaves exactly as before.
- **A row action's `visibleWhen` naming an endpoint no longer hides the action.** `CnRowActions` evaluates `visibleWhen` per row through the SYNC local evaluator, which answers `false` to anything naming an `endpoint` or a `source` because it cannot fetch. Nothing evaluated a row-level `visibleWhen` before, so a manifest already carrying one on `config.actions[]` lost that entry outright. The gate now runs only on a condition it can actually decide — a local `field` comparison, or a composition of them — and an endpoint/source condition falls through to visible, the same fail-safe posture the evaluator takes for a missing condition. `utils/visibleWhen.js` exports `isLocallyDecidableVisibleWhen` for a surface doing the same. `CnFormPage` and `CnStatWidget` are unchanged: for a form field a condition that cannot be decided still hides. Two more fixes in the same evaluator: a malformed `{ all: {}, any: [] }` hides one element instead of throwing out of the computed and taking the render with it, and the right-hand side of a local comparison now resolves `@object.<field>` against the record in hand, so a condition can compare one field with another rather than with nine literal characters.

### Added
- **A widget placement can publish itself to the native Nextcloud dashboard.** A v2 manifest's `pages[].widgets[]` entries take an optional `ncDashboard` object, so a placement can also appear as a panel on Nextcloud's own dashboard rather than only inside the app's pages. An object and not a boolean because a Nextcloud panel is a single narrow column carrying its own `title`, `icon`, `order` and `link`, none of which the grid placement has. Declaring it requires an `id` on the same entry, unique across the whole manifest — the host derives a permanent Nextcloud widget id from it, and that namespace is per app and out of reach of any migration this app could run. **Consumer impact:** additive; a placement without `ncDashboard` validates exactly as before. Schema 2.41.0.
- **`CnCapabilityTable` renders a capability comparison the host app supplies.** A debounced search over a row's number, name, area, feature and provider in both languages at once; a toggle between grouping by feature and grouping by area; and a provided-by column saying whether this app, a sibling app or the platform delivers each row — because a capability Nextcloud already provides is not a gap, and a table that cannot say so understates the product. No row is ever dropped: an unrecognised provider key shows raw, an undeclared area or feature gets a group of its own, a row naming no feature lands under a heading that says so, a rating outside the three the audit emits counts as unknown, and a repeated or missing `id` costs a row nothing. `CnFeaturesAndRoadmapView` and `CnFeaturesAndRoadmapPage` take the document through an optional `capabilityComparison` prop (the page also reads `features_roadmap_capabilities` off initialState) and grow a third stop on the header toggle. **Consumer impact:** none; pass nothing and both render exactly what they rendered before.
- **A manifest handler can live in the v2 registry, so an app can stop passing `customComponents` entirely.** `customComponents` has been deprecated for v2 manifests since ADR-036, but a manifest's NAMED BEHAVIOURS had nowhere else to go: `actions[].handler`, `bulkActions[].handler` and `headerActions[].handler` were read out of that map and nothing else, so an app with one function handler had to keep the whole deprecated map alive and wear the deprecation warning forever. The registry gains a `handler` kind for exactly that — a plain function exposed as `.handler` (or `.fn`), resolved by name and never mounted, so it carries no component and no metadata:

  ```js
  claimCase: { kind: 'handler', handler: claimCase },
  ```

  All three surfaces now resolve a name through `resolveRegisteredHandler`: the v2 registry first, then the legacy map. That resolver is the generalised body of `resolveCreateOverrideHandler`, which now delegates to it, so `config.createOverride` and an action handler cannot drift on what counts as a valid function. `CnIndexPage` injects `cnRegistry` for it.

  **Consumer impact:** none for an app that has not migrated — the legacy map is still read, and an app can move its handlers over one at a time. An app that registers a `kind: 'handler'` entry needs THIS release: an older library throws `RegistryKindError` on the unknown kind at mount, and, worse, silently resolves those handler names to nothing, so every migrated row action, bulk action and header action becomes a menu item that does nothing when clicked.

- **Saving a view says whether it worked.** Success closed the dialog and said nothing else, and a dialog closing is also what cancelling looks like — so the one gesture in the feature that writes to the server was the one with no confirmation that it had. A success toast now names the view; a failure toasts too AND keeps the dialog open with the reason, because the toast is what the person notices and the dialog is where the name and the public toggle still are to retry with. Neither toast can fail the save it reports on. A 2xx whose body carries no view counts as a failure and takes that path: axios throws on every non-2xx, so it is the one outcome nothing else marks, and the view is missing from the list either way — saying it worked would send the person looking for something that is not there. **Consumer impact:** none; a page that enables `allowSavedViews` gains the toasts.

- **An index page's filters, search and sort live in the route, so a view is a link.** `CnIndexPage` in self-fetch mode writes the active state into `$route.query` on every change and restores it on load, so a reload, a bookmark or a pasted URL reproduces the same list. **The non-underscore query namespace belongs to the page.** Reserved keys are `_`-prefixed (`_search`, `_order`, `_page`, `_limit`, `_tab`); every other key is read as a filter and sent to the API, which is what makes `?caseType=X&status=Y` a working deep link — and what means a host must prefix any param of its own with `_` to keep the page's hand off it. A key the page did not put in the query is left where it is rather than cleared on the next filter change. `@`-token deep links keep their SPELLING: `?assignee=@me` resolves to the signed-in user for the fetch, but the literal `@me` stays in the address bar for as long as it still resolves to the active filter, so a shared link keeps meaning "me" rather than freezing onto one named person.

### Changed
- **BREAKING: a list's sort travels as `_order` and nothing else, and Clear all now clears it.** Applying a saved view wrote the sort into the address as `_sortKey` / `_sortOrder`, a spelling nothing reads: `parseSortKeysFromQuery` seeds a list's sort from `_order` alone, `listNavigation` and `CnPageRenderer` parse the same shape, and OpenRegister's object API accepts `_order` and nothing else. So a saved view's sort survived only in memory — applying one looked right because the same branch set the list state directly, while a reload or the link sent to a colleague came back in the page's default order. Worse, the two keys sit on no reserved-key list, so they reached the API as filter FIELDS; in a consumer that builds a whole-result bulk selection off the query, that is two keys matching no column on the query deciding what the selection contains. `_order` is now the only spelling written or read, with no compatibility read of the flat pair. A view's stored `query.sort` becomes the ordered array `[{ key, order }, …]` so a chained sort survives a save; the single `{ key, order }` object every view stored before chaining is still read, so no stored view is lost. Clear all now clears the sort along with the filters and the search, because a sort an applied view set is one the reader did not choose and cannot see the origin of — left behind, it was state no control on the page could undo. **Consumer impact:** a link carrying `_sortKey` / `_sortOrder` opens in the page's default order; nothing writes that spelling any more, so only links already sent are affected. Clear all now resets the sort on every page, including one that has never shown a saved view.

### Removed
- **BREAKING: the `news` widget moves to LaunchPad, this time on the 2.x line.** `CnNewsWidget` and `CnNewsWidgetForm` are gone again, with the `news` type registration, its `libraryWidgetKeys` entry and its URL-prefix gate exemption. This is the second time: the removal first landed on 2026-09-11, semantic-release read its `BREAKING CHANGE:` footer as a major and published 3.0.0, and the removal was reverted eight days later so the fleet could get back onto a 2.x line it could actually install. Nothing was wrong with the removal — the revert said so itself, calling it "a real breaking change" that the tooling "did the right thing with". It came out because a major was unshippable, and that reason expired the same afternoon: the release config now computes a minor from a breaking footer and refuses any version outside 2.x, so this ships as a minor with its footer intact rather than as a 3. **Consumer impact:** the two exports no longer resolve, and an app rendering a stored `news` placement gets the unknown-type tile unless it registers its own. LaunchPad already owns the widget and its backend, and nothing in the fleet imports the library's copy — LaunchPad's only remaining mention is a docblock line recording where the code came from, and its test stub builds its own. The placement `content` shape is unchanged, so LaunchPad renders existing placements as before.
- **BREAKING: a saved view is a lens on a list, not a place with an address.** The whole `savedViewPlaces` capability is gone: the `src/utils/savedViewPlaces.js` module and its **sixteen barrel exports** (`pageHasSavedViewPlaces`, `savedViewRouteName`, `savedViewRoutePath`, `savedViewRouteBase`, `savedViewRouteTarget`, `viewIdForRoute`, `pinnedViewNavChildren`, `withPinnedViewChildren`, `isPinnedView`, `togglePinnedBy`, `resolveViewPresentation`, `PRESENTATION_VIEW_MODES`, `LEGACY_VIEW_QUERY_KEY`, `SAVED_VIEW_ROUTE_SUFFIX`, `DEFAULT_SAVED_VIEW_ROUTE_BASE`, `DEFAULT_PINNED_VIEW_CAP`); `CnIndexPage`'s `savedViewPlaces`, `savedViewId` and `savedViewRouteName` props; `CnSavedViewsControl`'s `allowPinning` prop and its `pin-request` event; the `savedViewPlaces` manifest key (schema **2.39.0 → 2.40.0**); and the `/<page-route>/<base>/:viewId` route records `buildManifestRoutes` emitted for it, along with the `?view=<id>` query it redirected. The route worked. What it wrote into the address did not — see the `_order` entry above — and a view stores filters, a search term and a sort, which is not enough to be somewhere you go. **The saved-views feature itself is untouched:** `allowSavedViews`, `CnSavedViewsControl`, `CnSaveViewDialog`, `useSavedViewsApi`, `savedViewsScope` and the tree/labels/inheritance helpers all stay, and applying a view still writes its state into the page's own query. `resolveViewPresentation` goes rather than moving, because its only caller was the route and `buildViewCreatePayload` has never written a `presentation`, so wiring it into the ordinary apply would have snapped every reader to the page's first view mode on every apply. **Consumer impact, three ways it lands:** an app importing any of the sixteen exports fails at build time with an unresolved import; a manifest still declaring `savedViewPlaces` on a page now fails validation, because `$defs.page` is `additionalProperties: false`; and a bookmarked `/<route>/views/<id>` URL no longer has a route record. **What to do instead:** delete the manifest key and the `allowPinning` binding — a view applies at the page's own address, which is the link to share. This ships inside a minor for the reason 2.54.0 records: the release config maps breaking to minor and refuses anything above 2.x, so the changelog is the notice.
...
semantic-release compute a major, and it published one. Then four more. This
package stays on the 2.x line, and the release config now refuses anything above
it.

**Nothing breaks in 2.54.0, and the news widget stays.** The removal that caused
all of this was reverted before publish. `CnNewsWidget` and `CnNewsWidgetForm`
still export, and the `news` type is still registered. This entry briefly said
the widget had moved to LaunchPad. It has not, and no consumer has to act.

**The 3.x versions are not deprecated yet.** `latest` points at 2.54.0, so any
range install gets the right thing. An exact pin on 3.0.0 through 3.4.0 still
resolves, and the `beta` tag still names 3.0.0-beta.5. Deprecating them needs a
token with publish rights, which this repository does not hold today. The
dispatch is ready in `.github/workflows/deprecate.yml`.
...
### Fixed
- **A `navigate` action to an external URL opens it, instead of landing on the app's fallback page.** `dispatchAction` handed every `navigate` target to `router.push`, and vue-router accepts an absolute URL as a *path* — so `https://www.youtube.com/watch?v=X` matched no route, fell through to the app's landing page, and carried the URL's own `?v=X` along with it, which read as the app swallowing the link. `CnActionButtons` now turns an external `navigate` into a real link: an anchor in the bar, an `NcActionLink` in a menu (`CnDetailPage` renders the `entries` channel's new `href` / `linkTarget` the same way), so the browser owns middle-click, "open in new tab" and the semantics assistive tech announces. The dispatcher keeps a `window.open` fallback for programmatic callers and no longer reaches the router with a URL. `isExternalActionTarget` is now exported from `utils/actionsDispatcher.js` and `CnIndexPage`'s row dispatcher, which already got this right, uses it instead of its private copy. **Consumer impact:** none for in-app paths; an external `navigate` that previously went nowhere now opens in a new tab.
- **`CnEditActionsModal` writes to an array the page actually renders.** It wrote `config.actions[]` on every page type. Only `CnIndexPage` declares an `actions` prop, and there it is the per-ROW menu; `CnDetailPage` and `CnDashboardPage` have no such prop at all — so an action added on a detail page was stored where nothing reads it and simply never appeared. The page-level surface all three render is `config.headerActions[]`, which is what the modal now edits by default. An index page, having both, gets a picker; every other type gets the Actions menu, its only option. Existing entries stranded under `config.actions` on a row-less page are reported with a one-click move into the menu, rather than silently relocated. **Consumer impact:** new actions land under `headerActions` instead of `actions`; nothing already stored is rewritten unless an author asks for the move.
- **`CnObjectSidebar` shows its subtitle.** The header's secondary line comes from `NcAppSidebar`'s `subname`; `subtitle` is only the `title` attribute on that line, and the line is not rendered at all while `subname` is empty. `CnObjectSidebar` passed only `subtitle`, so a manifest's `config.sidebar.subtitle` reached the component, was forwarded, and was invisible. It now passes both, the same pair it already passes for `name` / `title` and that `CnIndexSidebar` gets right. **Consumer impact:** a `subtitle` that was silently doing nothing now appears under the sidebar title.
- **A sidebar config edit takes effect without reloading the page.** Toggling the sidebar off applied to the manifest but left the panel on screen until a refresh, on both page types, because the manifest editors mutate `page.config.sidebar` IN PLACE — the object reference `CnPageRenderer` forwards never changes. `CnDetailPage` watched `sidebar` shallowly, so no edit to it ever fired a sync; it is now `deep`, like the `sidebarTabs` / `sidebarProps` watchers beside it (`assignSidebarState` already writes only keys that changed, so the extra calls cost a comparison). On `CnIndexPage` nothing watched the gate at all: under a `CnAppRoot` host `shouldRenderInlineSidebar` is false in both states, and `hoistedSidebarProps` carries neither `enabled` nor `show` — so a `hasSidebar` watcher now republishes the hoisted config. **Consumer impact:** none.
- **`CnEditSidebarModal` edits the sidebar the page actually has.** On an index page it did nothing: it offered tabs, which only `CnDetailPage` reads, and its visibility switch read `sidebar.show` — so a page with no sidebar config showed the switch already on while `CnIndexPage`, which mounts nothing without an explicit `sidebar.enabled === true`, rendered no sidebar. Nothing an author touched had any effect. The switch now reports the gate its page type reads, and the body follows the type. An index page gets the Search & columns panel's own settings — visibility, `showMetadata`, which tab opens first (`search.defaultTab`), and an editor for the extra `columnGroups[]` — with no Tabs section. A detail page keeps its tabs and gains the rest of what `CnDetailPage` reads off `config.sidebar`: the header `title` / `subtitle`, the `register` / `schema` the sidebar reads, and registry mode (`useRegistry` + `excludeIntegrations`), which swaps the five built-in tabs for one per registered provider — with the in-place warning that a declared `tabs[]` beats it, which `CnObjectSidebar` previously only logged to the console. `facets` stays out: it is the live facet data the page feeds the panel, not authored config. Per the v2 schema, `tabs[]` is the detail shape and `enabled` / `columnGroups` / `facets` the index one. **Consumer impact:** none, both stored shapes are unchanged.
- **`CnEditActionsModal` says it is empty, and picks icons from the browser.** A page with no actions yet opened as a blank modal, which reads as broken rather than as "nothing here yet" — the only way forward was the footer's "Add action", which nothing pointed at. It now shows a hint when the list is empty. The Icon field was also a free-text input, so an author had to already know a registry key such as `Trophy`; it is now a clearable `CnIconBrowser`, as in `CnPageConfigModal`. **Consumer impact:** none, the stored `config.actions[]` shape is unchanged.
- **A `CnPageConfigModal` switch redraws the first time it is flipped.** Turning "Show Cards / Table toggle" off applied the change but left the switch on, and pressing it again did nothing. `boolVal` probed the config with `Object.prototype.hasOwnProperty.call`, which goes through the `getOwnPropertyDescriptor` trap that Vue's reactive handlers do not implement — so reading a key the config did not have yet registered no dependency, and writing it triggered no re-render. It now reads the value through the proxy. This hit every toggle on its first click, since the modal only stores values that differ from the default. **Consumer impact:** none.
- **A dashboard tile shows its icon, colours and link.** A tile configured through the Add-widget modal rendered with no icon, a blue nobody chose, and a dead link — only its title came through. Three causes: `CnDashboardPage.getTileConfig` read the widget def's top level, where the modal writes only `title`, and now reads `content` first (the dual-shape read `CnDashTileWidget` already does); `CnTileWidget` treated an icon-registry key such as `Trophy` — what `CnIconBrowser` emits when no icon catalogue is wired — as a CSS class, and now renders it through `getIconComponent`, ahead of the `class` branch that still serves real `icon-*` classes; and the `tile` registry default no longer stores `#3b82f6`/`#ffffff` for an author who never opened the colour picker, with `CnTileWidget` painting no background at all in that case rather than a hardcoded Nextcloud blue. **Consumer impact:** a tile that never had colours set is now untinted — transparent, with ordinary text — instead of blue; explicit colours, preset tiles and legacy top-level tiles are unchanged.
- **`CnMarkdownEditor` toggles a format off through another format's markers.** Bold then italic then bold again produced `**_**text**_**`, because the toggle only looked at the delimiters immediately against the selection and a nested `_` hid the `**`. It now reads the whole run of known wrap markers on each side and matches this tool's pair at the same nesting depth. **Consumer impact:** none. Still open: the same blind spot when the markers are *inside* the selection (select all of `**_text_**`, press Ctrl+I).
- **`CnColorPicker` emits `input` again, so picking a colour does something.** `@ckpack/vue-color` emits `update:modelValue`, never `input`, and a consumer's listener was only being forwarded through `$attrs` — so every `@input="…$event.hex"` binding in the library's widget forms was inert. The event is now re-emitted and declared. **Consumer impact:** none, the documented `@input` payload is unchanged.
- **`CnPageRenderer` renders an in-app edit that creates a page's widget grid.** Adding a widget to a page with no grid yet updated the manifest but never appeared — the renderer held a one-shot `inject` snapshot of the pre-edit manifest. `CnAppRoot` now also provides `cnManifestSource` (the manifest as a ref); `CnPageRenderer` prefers it. **Consumer impact:** none, `cnManifest` is unchanged.
- **A stage a guard refuses now looks refused, and says why on a hover or a click.** Every stage a record could not reach rendered in the same grey, so on a live case the stage the case worker was aiming at was the faintest thing on the card. The guard's own sentence sat in small type beside the label of every one of them, and once more under the strip, which put the same words on screen four times.

  A refused move is now one state and it reads as one. `CnTimelineStages` takes `blocked` on a stage: it keeps full contrast, takes the Nextcloud warning colour, and shows an exclamation mark in its indicator, because orange against grey is exactly the pair a colour-blind reader cannot separate. Warning is the level, not error. A move a guard declines is a record working as its app intends, where a failed read of `/available-actions` is genuinely broken, and that one keeps the error colour so an admin can tell the two apart.

  Only the refused stage is coloured. A stage further down the process stays grey, and so does the stage the record is on. `CnTimelineStages` refuses the colour for the current stage even when a consumer asks for it: a refusal painted on the place the record already sits says no to a move nobody is making.

  The reason moved off the strip and onto three routes. The stage's `title` carries it for a mouse, a click or an Enter puts it in a warning note card under the strip for touch and keyboard, and a copy stays inside the stage so a screen reader announces it with no interaction at all. A reachable move's `description` is still printed: it says what will happen, and there is nowhere else for that to live.

  Two more things the same case page showed. A stage the record had already passed carried "Not possible from the current status" under a green, completed circle, which reads as something having gone wrong with something that already happened. History now says nothing, unless an action really does lead back to it. And every message became an `NcNoteCard`, the way the rest of this library reports one, instead of a paragraph of coloured text floating under a timeline.

  Vertical stages are top aligned, which was the other half of the report. A stage carrying a longer sentence than its neighbours is taller, and centring slid that one label and its circle out of line with the rest. The connector line was written to reach upwards by a percentage of the following stage's own height, so it only found the previous circle when every row was the same height. It now hangs off each stage and runs from that circle's centre to the next one down, both of them a fixed distance from the top of a row, so the line stays on the circles whatever a row contains. The horizontal variant is untouched: there the stage is a column, and its centring has nothing to do with vertical drift.
