...
### Fixed
- **Peer dependencies are no longer vendored into the Vue 3 dist.** `rollup.config.vue3.mjs` was missing the `external` entries that `rollup.config.js` has carried since the 2.20.1 singleton fix, so a dist built with it inlined four declared peers into `dist/esm/node_modules/`: `dexie`, `dompurify`, `marked` and `@vueuse/core` (measured against the published `2.1.0-vue3.16` tarball — 49 vendored packages, four of them peers). A vendored peer is a build-time snapshot frozen into the library while the consuming app resolves its own copy at install time, so the two drift apart on the next release and the app runs two different modules for one package, with ours winning inside our own components. `dexie` is the fatal case: its ESM entry claims a page-global singleton (`globalThis[Symbol.for("Dexie")]`) and throws at module evaluation when the versions differ — `Two different versions of Dexie loaded in the same app: 4.4.5 and 4.4.4` — so the bundle loads and the app mounts nothing. Those are the real numbers: the vue3 tarballs froze dexie 4.4.4 while declaring peer `^4.0.8`, which resolves to 4.4.5 today, so every consumer carried both. Apps hit it with exactly ONE dexie in their own lockfile, because the second copy was ours (pipelinq#1431, a dependabot 4.4.4 → 4.4.5 bump, failed its E2E boot gate this way; pipelinq#1455 repeated it for `marked`). `dompurify` made it a security defect rather than a packaging one — it is the XSS sanitizer, and a consumer could not patch it: they bump their own dependency, `npm audit` reports green, and the vulnerable vendored copy keeps running. `@vueuse/core` is declared across two majors (`^11.0.0 || ^14.0.0`), so the frozen major could differ from the app's and duplicated composables stopped sharing state. All four are now external, plus `gridstack` preventively — it is the defect this rule was first written for and is not reached by this entry graph today, but keeping both configs' lists identical is what stops the next divergence. **Consumers must supply these peers themselves**, which every app on the published main line already did; the compiled output is a bare specifier so the consumer's bundler resolves the single copy it already has
