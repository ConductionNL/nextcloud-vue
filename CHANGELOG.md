...
- **The stage strip on a record is now a widget, so the person who owns the page can place it, resize it or take it off.** dossiq's case page carried a hand-written transition strip across a whole row. Nobody but a developer could change it, it could not be moved, and the next app that wanted one had to write it again. `CnStagesWidget` is registered as the `stages` type on the detail-page surface, configured from the page itself by `CnStagesWidgetForm`.
  ...
  An optional availability endpoint gates each move. A blocked stage renders disabled with its reason on screen, because the reason is what tells the person what the record still needs, and it keeps its focus stop with `aria-disabled="true"` so a keyboard reaches it and hears why it is closed. When the availability read fails every move is blocked, since a guard that could not be checked is not a guard that passed. A move that declares a comment or a result opens `CnStageMoveDialog` before anything is sent, and the confirm button stays disabled until what the move requires is there.

  After a move the widget shows the new stage at once and fires `cn:page:refresh`, so the page re-reads the record and the availability answer for the new stage is refetched rather than assumed. `CnTimelineStages` learns a per-stage `disabled` flag for this, and emits no `stage-click` for one.
