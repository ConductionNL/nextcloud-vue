...
/**
 * Mount the page and let its record fetch settle; the body waits for it.
 *
 * @param {...any} args Forwarded to mountPage.
 * @return {Promise<object>} The mounted wrapper.
 */
async function mountSettled(...args) {
	const wrapper = mountPage(...args)
	await flushPromises()
	return wrapper
}

describe('CnDetailPage — the #widget-<id> slot carries the object context', () => {
	it('gives a custom widget the page objectId, not its empty default', async () => {
		const tab = (await mountSettled()).findComponent(CustomTab)
		expect(tab.exists()).toBe(true)
		expect(tab.props('objectId')).toBe('motion-42')
	})

	it('renders that id, so the widget can actually filter on it', async () => {
		expect((await mountSettled()).find('.custom-tab').text()).toBe('motion-42')
	})

	it('carries register / schema / objectType alongside the id', async () => {
		const tab = (await mountSettled()).findComponent(CustomTab)
	...
	it('supplies the id while the record is null', async () => {
		// The id comes from the route-fed prop, never read back off the record:
		// the fetch here yields no record (and no 404), so the body renders with
		// a null `resolvedObject` and the id must still reach the widget.
		const tab = (await mountSettled()).findComponent(CustomTab)
	...
	it('leaves a widget that declares none of the context props unaffected', async () => {
		const w = await mountSettled(ContextBlindTab)
