 ...
 */
const mountRoot = (manifest, registry = {}, routeName = 'Detail') => {
	return mount({
		render(h) {
			return h(CnAppRoot, {
				props: { manifest, registry, appId: 'testapp' },
			})
		},
	}, {
		global: {
			mocks: { $route: { name: routeName, params: {} } },
			stubs: { RouterView: true, NcContent: { template: '<div><slot /></div>' } },
		},
	})
}
...
describe('CnAppRoot page sidebar resolution', () => {
	it('renders the component a manifest page names', () => {
		const wrapper = mountRoot(
			...
			{ DetailSidebar: { kind: 'widget', component: SidebarStub } },
		)
	...
	it('resolves through the legacy customComponents map too', () => {
		const wrapper = mount({
			render(h) {
				return h(CnAppRoot, {
					props: {
						manifest: makeManifest({ sidebarComponent: 'DetailSidebar' }),
			...
			global: {
				mocks: { $route: { name: 'Detail', params: {} } },
				stubs: { RouterView: true, NcContent: { template: '<div><slot /></div>' } },
			},
		})
