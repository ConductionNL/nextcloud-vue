...
### D4 — Search + display cap, self-contained
`CnIconPicker` gets its own small substring matcher plus the dedupe-by-value + 120-item cap (uncapped while querying, always keeping the selected icon visible), ported from `filteredLimitedIconOptions`. **Why:** the FA solid pack alone is ~1000 icons — rendering all tiles is janky; the cap keeps first paint fast and search comprehensive. **Base note:** the chosen base branch (`beta`) does not yet carry `CnIconBrowser` (it lives on `development`), so this change does **not** import `CnIconBrowser/fuzzy.js` — it keeps its own matcher and its own catalogue-adapter module. This also avoids hard-coupling the two components; they can converge in a later change.
