...
- **Affected consumers:** All five fleet consumers (OpenRegister, OpenCatalogi, Procest, Pipelinq, LaunchPad) plus six pre-prod fleet apps (decidesk, docudesk, larpingapp, softwarecatalog, zaakafhandelapp, launchpad) — every host already mounts `CnAppRoot` so they automatically inherit the new defaults; no host changes required to keep current behaviour.
