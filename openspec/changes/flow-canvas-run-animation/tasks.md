...
- [x] Animator in `CnFlowDetail` — ordered step queue, bounded catch-up pace
...
- [x] Run-state rendering in `CnFlowDetail`'s node slot — halo pulse
      ...
      only.
- [x] Edge trace overlay in `CnFlowDetail`'s edge slot — success-coloured
      segment travelling the executed edge via `stroke-dashoffset` travel;
      quiet success stroke kept on executed edges. (The tree moved since this
      was written: `edgeGeometry()` is gone - Vue Flow routes the lines - so
      the trace rides CnFlowEdge's direction-pulse path over the same routed
      geometry, and the run classes travel on the edge records through the
