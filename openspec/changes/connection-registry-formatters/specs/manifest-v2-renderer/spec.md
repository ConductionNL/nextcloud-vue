...
- `connectionStatus(value)` SHALL render `configured` as "Configured", `limited` as "Limited", `unconfigured` as "Not configured", `simulated` as "Simulated", `disabled` as "Switched off", `unavailable` as "Not available" and `error` as "Error". Any other value SHALL pass through as `String(value)`. Null and undefined SHALL render as an empty string.
...
#### Scenario: A connection an admin switched off renders as switched off

- **WHEN** a column declares `{ key: "status", formatter: "connectionStatus" }` and a row carries `status: "disabled"`
- **THEN** the cell SHALL render "Switched off", or "Uitgeschakeld" for a Dutch reader
