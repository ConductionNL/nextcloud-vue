...
- [ ] Create `src/components/CnSupportDialog/CnSupportDialog.vue`:
...
- [ ] `index.js` re-export.
- [ ] JSDoc on every prop + event + computed (100% baseline).
- [ ] Scoped CSS only; cn- prefixed classes; Nextcloud CSS variables.

## Phase 2 — Composable

- [ ] Create `src/composables/useSupportDialog.js`:
...
- [ ] `CnFeaturesAndRoadmapSidebar.vue`:
...
- [ ] `src/components/index.js` — named export `CnSupportDialog`.
- [ ] `src/index.js` — named export (alphabetised).
- [ ] `src/composables/index.js` — named export `useSupportDialog`.
- [ ] `l10n/en.json` + `l10n/nl.json` — add new strings (component body, signature, four CTA labels, sidebar 4th container title + body + cta, updated "Four ways…" subtitle).
- [ ] `scripts/.jsdoc-baselines.json` — `"CnSupportDialog": 1`.

## Phase 5 — Tests

- [ ] `tests/components/CnSupportDialog.spec.js`:
...
- [ ] `docs/components/cn-support-dialog.md` — try-it, props/events tables, copy guidance, adoption recipe (`useSupportDialog` + first-open mount in `App.vue`).
- [ ] Cross-link from `docs/components/cn-features-and-roadmap-sidebar.md` (4th container).
