...
- **Apps that consume only the widget (e.g. `launchpad`)**: zero install-time changes. Bumping `@conduction/nextcloud-vue` to the version shipping this change is the entire integration. The widget no-ops gracefully if OR isn't installed/reachable.
