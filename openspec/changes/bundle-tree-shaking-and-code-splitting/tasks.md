...
- §2 Rollup chunking: DONE. ESM output → { dir:'dist/esm', preserveModules, preserveModulesRoot:'src' };
  CSS banner made a per-chunk fn so only the entry chunk emits `import './nextcloud-vue.css'`
  (a plain string banner emits a wrong relative path from nested chunks). CJS unchanged (single-file).
  package.json "module" → dist/esm/index.js. Build: dist/esm = 1669 chunk files (was one ~9.9MB file).
- §3 sideEffects: DONE (conservative allowlist) → ["*.css","./src/integrations/registry.js",
  "./dist/esm/integrations/registry.js"]. Swept src/: the ONLY module-top-level side-effect imports
  are CSS (index.js, utils/gridStack.js, CnDashboardGrid.vue — all matched by "*.css"); integrations
  registry builds its singleton at module top so it is kept. No consumer CSS/polyfill side-effect
  import can be tree-shaken away.
- §4 Lazy-load compiled v2 validator: DEFERRED — turns sync validateManifestV2 into async, a
  consumer-breaking API change rippling ~20 suites + every caller; §2+§3 already drop the validator
  chunk for consumers that never import it, so the bundle win is met without it. File a follow-up.
- §5 lodash removed (zero src imports). §5.2 npm install NOT run (node_modules symlinked); build+full
  suite green with it gone.
- Gate: npm run build EXIT 0; check:integration-build / check:integration-parity / check:docs all PASS.
  §1.2 (throwaway webpack consumer smoke) + §6.4 (live consumer tarball) DEFERRED (no consumer harness /
  no deploy to shared instance).
-->
