...
- [~] 1.1 Create `src/schemas/app-manifest.schema.json` with `version`, `menu[]`, and `pages[]` definitions; use closed enum for `type`; add `children[]` on menu items (max 1 level); add optional `config`, `component`, `headerComponent`, `actionsComponent` on page items — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 1.2 Generate `src/types/manifest.d.ts` from the JSON Schema (run once manually; commit the file for IDE support); add a `generate:types` npm script for future regeneration — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 2.2 Add async fetch inside `useAppManifest`: use `options.fetcher ?? axios` and `options.endpoint ?? generateUrl('/apps/{appId}/api/manifest')` as the target; deep-merge on 200; silently ignore non-200; run schema validation on merged result; revert to bundled + `console.warn` on failure — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 2.3 Create `tests/composables/useAppManifest.spec.js` with mocked axios via `options.fetcher`; test bundled-first, 200 merge, 404 fallback, invalid-manifest fallback, Options API compatibility, and custom endpoint option — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 3.2 Add slot override logic: resolve `page.headerComponent` and `page.actionsComponent` from `cnCustomComponents`; pass resolved components into `CnIndexPage`/`CnDetailPage` via scoped slots using dynamic slot definition — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 3.3 Write `CnPageRenderer.spec.js`; stub `CnIndexPage`, `CnDetailPage`, `CnDashboardPage`; provide mock manifest via `provide` AND via explicit prop; test each dispatch path, unmatched route, missing custom component, devtools name, name-only route matching, and standalone prop usage — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 3.4 Inspect `CnIndexPage.vue` and `CnDetailPage.vue`; if `#header` and/or `#actions` slots are missing, add them as empty scoped slots; verify with `npm test` that no existing test breaks — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 4.2 Add `permissions` prop (Array, default `[]`); filter menu items: item renders if `!item.permission` or `permissions.includes(item.permission)`; same filter applied to children — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 4.3 Write `CnAppNav.spec.js`; mock `$route`; provide mock manifest via `provide` AND via explicit `manifest` prop; test ordering, label resolution, permission filter, missing permissions prop, active route highlight (by route name), nested children, and standalone prop override — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 5.2 Write `CnAppRoot.spec.js`; verify each provide key is injected correctly by a child; test identity-t fallback; test missing customComponents default; test phase orchestration: loading phase shows #loading slot, dependency-missing phase shows #dependency-missing slot, shell phase shows #menu + router-view; test #menu slot override — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 6.1 Add `CnAppRoot`, `CnAppNav`, `CnPageRenderer`, `CnAppLoading`, `CnDependencyMissing` to `src/components/index.js`; add all five components plus `useAppManifest` and `useAppStatus` to `src/index.js` and `src/composables/index.js` — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 6.3 Add JSDoc to every prop, event, slot, and method across all new files (CnAppRoot, CnAppNav, CnPageRenderer, CnAppLoading, CnDependencyMissing, useAppManifest, useAppStatus); document the `t` prop → `cnTranslate` inject key mapping in CnAppRoot JSDoc; verify with `npm run lint` (no missing JSDoc warnings) — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 7.1 Create `tests/fixtures/manifest-valid.json` with all four page types represented; create `tests/fixtures/manifest-invalid.json` with a bad `type` value; write `tests/schemas/app-manifest.schema.spec.js` to validate both against the schema using Ajv — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 8.1 Create `examples/manifest-demo/manifest.json` illustrating all four page types, menu ordering, permission filtering, and nested children; add `examples/manifest-demo/README.md` showing the minimal `main.ts` wiring to `CnAppRoot` — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 10.1 Run `npm test` and fix any failures; run `npm run build` and verify named exports are present in the bundle; run bundle analysis to confirm `defineAsyncComponent` splits page type components — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 11.1 Create `CnAppLoading.vue` (Options API); props: `message` (String, default `'Loading...'`), `logoUrl` (String, default `''`); slot: `#logo`; template: centered layout with optional logo, `NcLoadingIcon`, and message text; Nextcloud CSS variables only; JSDoc props and slot — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 11.3 Create `src/composables/useAppStatus.js`; signature `useAppStatus(appId: string)`; use `@nextcloud/capabilities` (or OCS apps endpoint as fallback) and read the key matching `appId`; return `{ installed: Ref<boolean>, enabled: Ref<boolean>, loading: Ref<boolean> }`; cache results per `appId` in a module-level `Map` for the page lifetime; on error: `installed=false`, `enabled=false`, `console.warn`; export from `src/composables/index.js`; write `tests/composables/useAppStatus.spec.js` covering cache-per-appId, missing capability, and error path — deferred to downstream cycle / fleet-wide adoption (handoff)

---

## Verification

- [~] All tasks checked off — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] `npm test` passes (zero failures, zero skipped) — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] `npm run build` succeeds with all seven new exports present (`CnAppRoot`, `CnAppNav`, `CnPageRenderer`, `CnAppLoading`, `CnDependencyMissing`, `useAppManifest`, `useAppStatus`) — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] JSON Schema validates all fixture manifests correctly (including semver pattern and `$schema` field) — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] No `cnT` references remain anywhere in the four artifact files (all replaced with `cnTranslate`) — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] CnPageRenderer matches routes by `$route.name === page.id` only (no path matching) — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] Manual test: mount CnAppRoot in a Nextcloud app dev environment and verify loading phase, dependency-missing phase, and shell phase for all four page types — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] Manual test: mount CnPageRenderer standalone (no CnAppRoot) with explicit `manifest` prop — pages render correctly — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] Manual test: CnAppRoot #menu slot override — custom menu component renders instead of CnAppNav — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] CLAUDE.md updated and accurate — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] No `--nldesign-*` variable references in any new CSS — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] JSDoc complete on all public props, events, slots, and methods — deferred to downstream cycle / fleet-wide adoption (handoff)
