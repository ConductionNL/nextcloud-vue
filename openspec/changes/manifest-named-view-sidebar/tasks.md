...
- [x] Add a reactive holder `pageSidebarComponent: { value: null }`
      to `CnPageRenderer.data()` mirroring `pageSidebarVisible`.
- [x] Add `cnPageSidebarComponent` to the `provide()` return so
      descendant injects observe holder mutations.
- [x] Add a `currentPageSidebarComponent` computed that resolves
      ...
      id) and return `null`.
- [x] Watch `currentPageSidebarComponent` and push the value into
      `pageSidebarComponent.value` on the `immediate: true` flush,
      same shape as the existing visibility watcher.
- [x] When BOTH `sidebar.show: false` and `sidebarComponent` are
...
- [x] Add `cnPageSidebarComponent` to `CnAppRoot.inject` with a
      default of `() => ({ value: null })` (so apps without a
      `CnPageRenderer` ancestor pass through unchanged).
- [x] Update the `#sidebar` slot block to include default content
...
- [x] Keep the visibility gate (`v-if="cnPageSidebarVisible.value
...
- [x] Update `src/utils/validateManifest.js` to enforce
...
- [x] `tests/components/CnPageRendererNamedViewSidebar.spec.js`
      (new) — covers:
...
- [x] Update `tests/components/CnAppRoot.spec.js` (or add a new
...
- [x] Update `tests/utils/schema.spec.js` (or
...
- [x] Update `docs/migrating-to-manifest.md` — add a "Per-route
...
- [x] Run `npm run check:docs` and `npm run check:jsdoc`. Both
      MUST pass.

## Phase 6 — Spec capture

- [x] Write `specs/manifest-named-view-sidebar/spec.md` with
