...
OpenBuild — the upcoming Nextcloud app builder — breaks both assumptions. It mounts virtual
...
no backend route (the builder is a frontend-only mount of arbitrary JSON). Today, OpenBuild's
...
That round-trip is a temporary bridge (see openbuild's `bootstrap-openbuild/design.md`,
...
- Migrating OpenBuild's `BuilderHost.vue`. That migration is a follow-up in the openbuild
...
malformed manifests at mount time. OpenBuild's in-memory manifests come from the builder UI,
...
backend override path. Virtual-app hosts like OpenBuild want a direct mount with no IO. A
...
- Should we emit a `console.warn` when consumers use the *old* positional signature with a
  redirected `options.endpoint` (the OpenBuild-style workaround)? Provisional answer: **no**.
...
- Should the in-memory branch accept an `appId` in the options object for logging /
  ...
  nothing to defer. The current OpenBuild consumer puts its slug inside the manifest itself
...
3. Should the composable emit a deprecation warning when `options.endpoint` is used in the
   OpenBuild-style redirection pattern? Provisional answer in the design body is "no" —
