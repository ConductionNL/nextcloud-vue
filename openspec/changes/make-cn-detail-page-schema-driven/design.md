...
- Backwards-compat: every existing `CnDetailPage` consumer (call sites in OpenRegister, OpenCatalogi, Procest, Pipelinq, MyDash, decidesk, openbuild itself for non-manifest routes) must keep working unchanged.
...
6. Sweep the fleet — every app already on the manifest pattern (decidesk, mydash, openbuild, plus the in-flight journeydoc rollouts) inherits the behaviour. No per-app code change required; per-app `npm i` + redeploy is enough.
