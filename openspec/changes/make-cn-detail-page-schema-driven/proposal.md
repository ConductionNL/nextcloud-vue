...
- **Affected consumers (immediate)**: openbuilt (`VirtualAppDetail` route works without a custom view), decidesk + launchpad (existing detail routes keep working — auto-widgets only fire on empty slot).
