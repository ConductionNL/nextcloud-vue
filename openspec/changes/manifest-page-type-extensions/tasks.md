...
- [x] Update `src/schemas/app-manifest.schema.json` to extend the
...
- [x] Update `src/utils/validateManifest.js` test fixtures to cover
      the new enum values + each new type's required `config` fields.
- [x] Add per-type `config` shape validation:
...
- [x] Add `src/components/CnLogsPage/CnLogsPage.vue` — wraps
...
- [x] Add `src/components/CnSettingsPage/CnSettingsPage.vue` — renders
...
- [x] Each new component MUST honour `headerComponent`,
...
- [x] Update `src/components/CnPageRenderer/CnPageRenderer.vue` to
...
- [x] Open per-app follow-up issues for migrating existing
