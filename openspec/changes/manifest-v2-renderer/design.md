...
**Rationale:** ADR-036 Decision 8 explicitly states "does not merge". Runtime manifests are the canonical source of truth (launchpad, OpenBuilt); merging would silently override user-authored layouts with stale bundled defaults — a correctness hazard. The no-merge contract simplifies reasoning about manifest state.
