...
- [x] 1.1 `useTaskInboxStore` (internal): one `load(config)` against
...
- [x] 1.2 `tasks` in `indexSources.js`: row mapping (`id`, `title`,
...
- [x] 3.1 `CnTasksWidget`: endpoint binding (`isTerminal=false`, scope,
      ...
      and shape.
- [x] 3.2 Quick actions: claim on pooled rows, complete (per outcome when
      declared) on own rows, refusals as toasts with the server's message,
      refetch after every verb.
- [x] 3.3 `CnTasksWidgetForm` + registration: type key `tasks`, defaults,
      `CONTENT_ONLY_TYPES`, `registerDashboardWidgets`, barrel exports.

## 4. Schema, l10n, docs

- [x] 4.1 `entitySource` enum gains `tasks` in
      `src/schemas/app-manifest-v2.schema.json` (+ schema spec test).
- [x] 4.2 New strings in `l10n/en.json` and `l10n/nl.json`.
- [x] 4.3 `docs/components/cn-tasks-widget.md`, `cn-tasks-widget-form.md`;
...
- [x] 5.1 Unit: source registration, request params and allowlist, row
      mapping, columns, quick-filter reloads and the single mount load,
      openRow, renderer `sourceConfig` bridge, schema enum.
- [x] 5.2 Unit: widget states, honest count and remainder, polling, per-row
      action visibility, claim/complete posts, refusal toast wording, form
      round-trip.
- [x] 5.3 Playwright (offline harness): inbox rows via the index page, pool
