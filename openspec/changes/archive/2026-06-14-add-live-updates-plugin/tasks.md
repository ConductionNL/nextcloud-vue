...
- [x] 2.1 Create `src/store/liveUpdates/eventKeys.js` with constants and builder functions
...
- [x] 3.1 Create `src/store/liveUpdates/websocketTransport.js` with singleton state, refcounted listener map, and reconnect backoff
...
- [x] 4.1 Create `src/store/liveUpdates/pollingTransport.js` with refcounted intervals and `visibilitychange` listener
...
- [x] 5.1 Create `src/store/liveUpdates/transport.js` implementing `getLiveUpdates()` factory with transport selection and status observer
...
- [x] 6.1 Create `src/store/plugins/liveUpdates.js` implementing the full plugin factory
...
- [x] 7.1 Add dedup Maps to liveUpdatesPlugin `setup()` hook; wrap `fetchObject` and `fetchCollection` store actions to check/populate the maps before making HTTP calls
...
- [x] 8.1 Add `export { liveUpdatesPlugin } from './liveUpdates.js'` to `src/store/plugins/index.js`; re-export through `src/store/index.js` and `src/index.js`
...
- [x] 9.1 Add the `**OpenSpec changes**` line to `openspec/specs/store/spec.md` near the Purpose section
...
- **acceptance_criteria**:
  - GIVEN `buildObjectKey('uuid-abc')` THEN returns `'or-object-uuid-abc'`
  - GIVEN `buildCollectionKey('zaken', 'meldingen')` THEN returns `'or-collection-zaken-meldingen'`
- [x] 10.1 Create unit tests for eventKeys module
...
- [x] 10.2 Create unit tests for liveUpdatesPlugin covering subscribe, unsubscribe, dedup, and status transitions
...
- [x] No hardcoded event key strings outside `eventKeys.js`
- [x] No direct references to `@nextcloud/notify_push` outside `websocketTransport.js` and `transport.js`
