...
Add a declarative `lifecycleActions` config block to `type:"detail"` pages
so manifest authors can render status-gated lifecycle/transition buttons
driven by OpenRegister's `x-openregister-lifecycle`, without hand-coding an
action endpoint + button row per schema. The capability is additive: a
detail page that omits `lifecycleActions` behaves exactly as before.
