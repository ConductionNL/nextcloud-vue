...
Add three additive `type:"detail"` config blocks to `CnDetailPage` —
`relatedCollections`, `summaryAggregates`, and `relationLinks` — so a manifest
author can declare related-object list sections, cross-schema summary chips,
and a search-and-link relation action without hand-coding a custom page. Each
block defaults to empty and changes nothing when omitted.
