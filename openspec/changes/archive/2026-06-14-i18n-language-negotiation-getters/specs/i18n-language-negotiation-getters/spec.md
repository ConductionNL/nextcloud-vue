...
Provide frontend i18n primitives in `@conduction/nextcloud-vue` that
consume OpenRegister's shipped `i18n-api-language-negotiation` (read
side: `?_lang=<bcp47>`) and `i18n-source-of-truth` (write side:
`X-Translation-Target-Language: <bcp47>`) contracts. Closes the
read- and write-side language gaps catalogued by the larpingapp and
procest OR-abstractions audits (§3.2 / §3.3 in both proposals).
