...
**Scope:** nextcloud-vue
**Tier:** V1
**Depends on:** `CnWizardDialog`, `fieldsFromSchema`, `CnFormDialog`, `CnSettingsCard`, `CnConfigurationCard`, `CnSuggestFeatureModal`, `CnActionsMenu`, `CnAppRoot`, `CnAdminSettingsShell`; the `manifest-setup` schema; hydra ADR-042.

## Purpose

Provide the abstract setup wizard component, the status composable, the CnAppRoot
gating phase, and the admin-page entry that together render a manifest `setup`
block, gate the app until required steps are met, and run privileged work through a
server-side contract (never a browser write).
