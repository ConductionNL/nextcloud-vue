...
Fleet-wide saved-search-views UI on `CnIndexPage`, consuming OpenRegister's existing Views backend (`/apps/openregister/api/views`). Opt-in per page via the `allowSavedViews` prop; state is exchanged through CnIndexPage's route-query deep-link contract.
