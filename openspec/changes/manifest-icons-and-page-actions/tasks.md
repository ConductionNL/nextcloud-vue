...
- [~] 1.1 Implement iconClass prop with JSDoc + wrapping div in CnStatsBlockWidget.vue — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 1.2 Extend `getStatsBlockProps` allowlist to include `iconClass` — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 1.3 Add tests covering both forwarding paths — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 2.1 Add headerActions prop, render loop, click → emit wiring — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 2.2 Add headerActions prop, mergedHeaderActions computed, resolveHeaderHandler method, onHeaderAction emit handler — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 2.3 Bind `:header-actions="mergedHeaderActions"` and `@header-action="onHeaderAction"` in template — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 2.4 Add `headerActions` property to `pages[].config` in both schemas, referencing the existing `action` $def — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 2.5 Add tests covering all dispatch paths and back-compat — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 3.1 Add JSDoc to `iconClass`, `headerActions` props and `@header-action` event — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 3.2 Run prebuild:docs and commit the regenerated files — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 3.3 Author / extend the hand-written .md pages — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] 4.1 Run gate suite and fix any failures — deferred to downstream cycle / fleet-wide adoption (handoff)
