...
- [ ] 1.1 Implement iconClass prop with JSDoc + wrapping div in CnStatsBlockWidget.vue
...
- [ ] 1.2 Extend `getStatsBlockProps` allowlist to include `iconClass`
...
- [ ] 1.3 Add tests covering both forwarding paths
...
- [ ] 2.1 Add headerActions prop, render loop, click → emit wiring
...
- [ ] 2.2 Add headerActions prop, mergedHeaderActions computed, resolveHeaderHandler method, onHeaderAction emit handler
...
- [ ] 2.3 Bind `:header-actions="mergedHeaderActions"` and `@header-action="onHeaderAction"` in template
...
- [ ] 2.4 Add `headerActions` property to `pages[].config` in both schemas, referencing the existing `action` $def
...
- [ ] 2.5 Add tests covering all dispatch paths and back-compat
...
- [ ] 3.1 Add JSDoc to `iconClass`, `headerActions` props and `@header-action` event
...
- [ ] 3.2 Run prebuild:docs and commit the regenerated files
...
- [ ] 3.3 Author / extend the hand-written .md pages
...
- [ ] 4.1 Run gate suite and fix any failures
